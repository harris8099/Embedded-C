...
/*
 * ARM Cortex Mx processor number of priority bits implemented in Priority registor
 */
...
#define ROM 								0x1FFF0000U		/*Base address of the ROM or System memory*/
#define SRAM 								SRAM1_BASEADDR	/*Use SRAM macro as SRAM1*/


//base address for bus interfaces
...
typedef struct
{
	__vo uint32_t MODER; // configure the I/O direction mode
	__vo uint32_t OTYPER; // configure the output type of the I/O port
	__vo uint32_t OSPEEDR; // configure the I/O output speed
	__vo uint32_t PUPDR; // configure the I/O pull-up or pull-down
	...
	__vo uint32_t BSRR;
	__vo uint32_t LCKR;
	__vo uint32_t AFR[2]; // AFR[0] : alternate function low register
...
typedef struct{
	...
	__vo uint32_t RSTR;		/*<  Address offset 0x08>*/
	__vo uint32_t FSTR;		/*<  Address offset 0x0C>*/
